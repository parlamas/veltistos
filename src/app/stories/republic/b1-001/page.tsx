//src/app/stories/republic/b1-001/page.tsx
...
type Block = {
  menu?: string;
  label: string;
  html?: string;   // for translations
  htmlId?: string; // for attic container id
  tts: string;
  lang: string;
};
...
const blocks: Record<string, Block> = {
  ...
  es: {
    ...
    tts:
      "Bajé ayer al Pireo con Glaucón, hijo de Aristón, para rezar a la diosa y también porque quería ver cómo iban a celebrar la fiesta, ya que la celebraban por primera vez. Me pareció buena la procesión de los lugareños, pero no menos adecuada fue la que organizaron los tracios.",
    lang: "es-ES",
  },
  fr: {
    menu: "Français",
    ...
    tts:
      "Je suis descendu hier au Pirée avec Glaucon, fils d’Ariston, pour prier la déesse et aussi parce que je voulais voir comment ils allaient célébrer la fête, puisqu’ils la célébraient pour la première fois. La procession des habitants m’a paru belle; celle des Thraces ne l’était pas moins.",
    lang: "fr-FR",
  },
  da: {
    menu: "Dansk",
    ...
    tts:
      "Jeg gik i går ned til Piræus sammen med Glaukon, Aristons søn, for at bede til gudinden og også fordi jeg gerne ville se, hvordan de ville holde festen, eftersom de holdt den for første gang. Processionen for de lokale forekom mig god, men ikke mindre passende var den, som thrakerne stod for.",
    lang: "da-DK",
  },
  de: {
    menu: "Deutsch",
  ...
  pl: {
    ...
    tts:
      "Wczoraj zszedłem do Pireusu z Glaukonem, synem Aristona, aby pomodlić się do bogini, a także dlatego, że chciałem zobaczyć, jak będą obchodzić święto, skoro obchodzili je po raz pierwszy. Procesja miejscowych wydała mi się dobra, lecz nie mniej stosowna była ta, którą urządzili Trakowie.",
    lang: "pl-PL",
  },
  it: {
    menu: "Italiano",
    ...
    tts:
      "Ieri sono sceso al Pireo con Glaucone, figlio di Aristone, per pregare la dea e anche perché volevo vedere come avrebbero celebrato la festa, dato che la tenevano per la prima volta. La processione degli abitanti mi parve bella, ma non meno adeguata fu quella organizzata dai Traci.",
    lang: "it-IT",
  },
  fi: {
    menu: "Suomi",
  ...
  el: {
    ...
    html: `
Κατέβηκα<sub>1</sub> χθες<sub>2</sub> στον<sub>3</sub> Πειραιά<sub>4</sub> μαζί<sub>5</sub> με<sub>6</sub> τον<sub>7</sub> Γλαύκωνα,<sub>8</sub> 
τον<sub>9</sub> γιο<sub>10</sub> του<sub>11</sub> Αρίστωνα,<sub>12</sub> και<sub>13</sub> για<sub>14</sub> να<sub>15</sub> προσευχηθώ<sub>16</sub> 
στη<sub>17</sub> θεά<sub>18</sub> αλλά<sub>19</sub> και<sub>20</sub> επειδή<sub>21</sub> ήθελα<sub>22</sub> να<sub>23</sub> δω<sub>24</sub> με<sub>25</sub> 
ποιόν<sub>26</sub> τρόπο<sub>27</sub> θα<sub>28</sub> τελέσουν<sub>29</sub> την<sub>30</sub> γιορτή,<sub>31</sub> αφού<sub>32</sub> τώρα<sub>33</sub> την<sub>34</sub> 
έκαναν<sub>35</sub> για<sub>36</sub> πρώτη<sub>37</sub> φορά.<sub>38</sub> Μου<sub>39</sub> φάνηκε<sub>40</sub> λοιπόν<sub>41</sub> ωραία<sub>42</sub> η<sub>43</sub> 
πομπή<sub>44</sub> των<sub>45</sub> εντοπίων,<sub>46</sub> ωστόσο<sub>47</sub> εξ’ίσου<sub>48</sub> όμορφη<sub>49</sub> μου<sub>50</sub> φάνηκε<sub>51</sub> και<sub>52</sub> 
η<sub>53</sub> πομπή<sub>54</sub> των<sub>55</sub> Θρακών.<sub>56</sub>
<br>
<audio controls>
  ...
  `,
    tts:
      "Κατέβηκα χθες στον Πειραιά μαζί με τον Γλαύκωνα, τον γιο του Αρίστωνα, και για να προσευχηθώ στη θεά αλλά και επειδή ήθελα να δω με ποιον τρόπο θα τελέσουν τη γιορτή, αφού τώρα την έκαναν για πρώτη φορά. Μου φάνηκε ωραία η πομπή των εντοπίων, ωστόσο εξ’ίσου όμορφη μου φάνηκε και η πομπή των Θρακών.",
    lang: "el-GR",
  },
};
...
// Attic HTML block (as in your file)
const ATTIC_INNER_HTML = `
<div class="blue">
...
// Utilities
// ------------------------------------------------------------------------------------
function pickVoice(forLang: string) {
  const synth = window.speechSynthesis;
  const voices = synth.getVoices();
  if (!voices || !voices.length) return null;
  return (
    voices.find((v) => v.lang === forLang) ||
    voices.find(
      (v) => v.lang && v.lang.toLowerCase().startsWith(forLang.split("-")[0])
    ) ||
    voices[0] ||
    null
  );
...
async function speak(kind: "attic" | "translation", selectedKey: string | null) {
  ...
  if (kind === "attic") {
    ...
    text = blocks[selectedKey].tts;
    lang = blocks[selectedKey].lang;
  }

  await new Promise((r) => setTimeout(r, 100)); // small delay for Safari
...
// Page component
// ------------------------------------------------------------------------------------
export default function SP001Page() {
  const [selected, setSelected] = useState<string | null>(null);
  const atticRef = useRef<HTMLDivElement | null>(null);

  // Safari: ensure voices load
  useEffect(() => {
    if ("speechSynthesis" in window) {
  ...
  // Stop buttons (global) — cancel ongoing speech
  useEffect(() => {
    function onDocClick(e: MouseEvent) {
      const target = e.target as HTMLElement | null;
      if (!target) return;
      const stopBtn = target.closest(".stopper");
      if (!stopBtn) return;
      if ("speechSynthesis" in window) {
  ...
  // Scroll to Attic card on language change
  useEffect(() => {
    if (!selected) return;
    const top = atticRef.current?.offsetTop ?? 0;
    window.scrollTo({ top, behavior: "smooth" });
    // Cancel any ongoing speech when switching
    if ("speechSynthesis" in window) window.speechSynthesis.cancel();
  }, [selected]);

  const isModernGreek = selected === "el";

  return (
    <main>
      {/* Optional external CSS (as in your HTML). If you use Next layouts, move this to <Head>. */}
      <link rel="stylesheet" href="q.css" />

      {/* Page-level styles (ported from your <style> tag) */}
      <style jsx global>{`
        :root {
          --bg: #f8fafc;
          --card: #ffffff;
      ...
      <div className="container">
        <h2 className="title">
          <span style={{ color: "brown" }}>
            The Birth of Dialectics and Horistics
          </span>
          <hr />
          <a style={{ textDecoration: "none" }} href="https://horistics.ai">
            horistics.ai
          </a>{" "}
          •{" "}
          <a style={{ textDecoration: "none" }} href="https://horistics.com">
            horistics.com
          </a>{" "}
          •{" "}
          <a style={{ textDecoration: "none" }} href="https://veltistos.com">
            veltistos.com
          </a>
          <hr />
          η Πολιτεία τοῦ Πλάτωνα • Plato&apos;s Republic
        </h2>
        <div className="subtitle">
          Parallel viewer — Attic Greek + one selected translation with on-page
          TTS
        </div>

        {/* Attic card */}
        <div className="card" id="attic-card" ref={atticRef}>
          <div className="row">
            <div className="lang-badge">
              Book 1, Part 1 • Πρωτότυπο — Ἀττικὴ διάλεκτος • Original Text —
              Attic Greek <hr />
              Σωκράτης
            </div>
            {/* (Attic Listen/Stop were commented out in your HTML; keeping them omitted here) */}
          </div>

          <div className="content" id="attic-html">
            <div dangerouslySetInnerHTML={{ __html: ATTIC_INNER_HTML }} />
          </div>
        </div>
        ...
        >
          <div className="row">
            <div className="lang-badge" id="trans-label">
              {selected ? blocks[selected].label : "\u00A0"}
            </div>

            {/* Listen button (hidden for Modern Greek) */}
            {!isModernGreek && (
              <button
                className="speaker"
                data-say="translation"
                type="button"
                onClick={() => speak("translation", selected)}
              >
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
            ...
            {/* Stop button (hidden for Modern Greek) */}
            {!isModernGreek && (
              <button className="stopper" data-stop="attic" type="button">
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: selected ? blocks[selected!].html || "" : "",
            }}
          />
        </div>
      </div>
